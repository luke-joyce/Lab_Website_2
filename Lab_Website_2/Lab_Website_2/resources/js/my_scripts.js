...
function xx(id, toggle){
	if(toggle == 0)
		document.getElementById(id).style.visibility=false;
...
function loadStatsPage(){
	...
	for(i=2; i<table.rows.length; i++) {
		home=parseInt(table.rows[i].cells[2].innerHTML);
		visitor=parseInt(table.rows[i].cells[3].innerHTML);
		if(home>visitor) {
			table.rows[i].cells[4].innerHTML="CU";
			wins++;
		}
		if(home<visitor) {
			var teamName = table.rows[i].cells[1].innerHTML;
			table.rows[i].cells[4].innerHTML=teamName;
			losses++;
		}
	}
...
function loadPlayersPage()
{
	...
	{
		field+='<a href="#' + players[i].name + '" onclick = switchPlayers(' + i + ')>' + players[i].name + '</a><br>';
	}
	document.getElementById("player_selector").innerHTML = field;
}				
